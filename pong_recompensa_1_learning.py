...
def rewardFunction( ball , paddle1, ballDir):
    global ERROS
    
    ##    if ball.left  > LINETHICKNESS+PADDLEOFFSET:
    ##        if ballDirY > 0:
    ##            if acao == "down":
    ##                R = 1
    ##            elif acao == "up":
    ...
    # print "paddle1.right: ",paddle1.right
    # print "ballDirX", ballDirX
    # print "ballDirY", ballDirY
...
def CALCQ( ball , angle , paddle1 , ACTION, ballDir):
    ...
    + ALPHA*(rewardFunction( ball , paddle1, ballDir) + GAMMA*( MAX_NEXT_STATE(X,Y,angle,paddle1, ACTION) -  Q[ball.x][ball.y][angle][paddle_pos][ACTION][0]) ) )
